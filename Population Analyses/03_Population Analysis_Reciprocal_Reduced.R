...
####             Research
####
#### Script: Population analysis: additional analysis
####         Model without reciprocal effects -> reduced effects of auto and cross
...
# linear increase
multi_scenario3_List <- lapply(1:5, FUN = function(i) rbind(rep(1 + .075*(i-1), 100),
                                                            rep(1 - .2*(i-1), 100)))
...
multi_scenario4_List <- lapply(1:5, FUN = function(i) rbind(rep(1 + .03*i*(-1.2)^(i-1)-.03, 100),
...
set.seed(1234)
multi_scenario11_List <- lapply(1:5, FUN = function(i) rbind(runif(n = 100, min = 0, max = 2.2),
...
temp <- left_join(SmallestEigenValues,
                  reshape(aggregate(R2 ~  R2_name + Scenario, data = dataR2, max), 
                          idvar = "Scenario", timevar = "R2_name", direction = "wide"), 
                  by = "Scenario")
temp


subset_data <- subset(dataR2, Scenario != "Scenario 0")
temp <- aggregate(R2 ~ R2_name, data = subset_data, range)
min(temp$R2[1:2,1]) # R2_C
max(temp$R2[1:2,2]) # R2_C

min(temp$R2[5:6,1]) # R2_incr
max(temp$R2[5:6,2]) # R2_incr

min(temp$R2[3:4,1]) # R2_total
max(temp$R2[3:4,2]) # R2_total
...
save(list = c("Results", "SmallestEigenValues", "dataR2", 
              ...
              "StepOne_10",
              "StepOne_11"),
     file = "00_Data/03_PopulationAnalysis_Reciprocal_Reduced.rda")
