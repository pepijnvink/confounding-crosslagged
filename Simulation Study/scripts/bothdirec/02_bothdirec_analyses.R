...
data_s1 <- readRDS(paste0("output/bothdirec/simdat/N", N, "/scenario1_N", N, ".rds"))
...
saveRDS(data_s1, file = paste0("output/bothdirec/analyzed_dat/N", N, "/analyzed_scenario1_N", N, ".rds")) # analyzed data
...
data_s1 <- remove_invalid(data_s1)
...
saveRDS(res_s1, file = paste0("output/bothdirec/results/N", N, "/results_scenario1_N", N, ".rds")) # save results
# Scenario 2
data_s2 <- readRDS(paste0("output/bothdirec/simdat/N", N, "/scenario2_N", N, ".rds"))
data_s2 <- analyze_data(data_s2, models = list(clpm = clpm, # analyze data
                                               ...
                                               dpm = dpm,
                                               dpm_free = dpm_free))
saveRDS(data_s2, file = paste0("output/bothdirec/analyzed_dat/N", N, "/analyzed_scenario2_N", N, ".rds")) # analyzed data
...
data_s2 <- remove_invalid(data_s2)
res_s2 <- output_measures(data_s2, xyvar = output_vars) # output measures (bias, mse etc.)
saveRDS(res_s2, file = paste0("output/bothdirec/results/N", N, "/results_scenario2_N", N, ".rds")) # save results
rm(data_s2)

# Scenario 3
data_s3 <- readRDS(paste0("output/bothdirec/simdat/N", N, "/scenario3_N", N, ".rds"))
data_s3 <- analyze_data(data_s3, models = list(clpm = clpm, # analyze data
                                               ...
                                               dpm = dpm,
                                               dpm_free = dpm_free))
saveRDS(data_s3, file = paste0("output/bothdirec/analyzed_dat/N", N, "/analyzed_scenario3_N", N, ".rds")) # analyzed data
#plan(sequential) # back to sequential processing
data_s3 <- check_convergence(data_s3, latent = c(F, F, T, T, T, T)) # check convergence of models
data_s3 <- remove_invalid(data_s3)
res_s3 <- output_measures(data_s3, xyvar = output_vars) # output measures (bias, mse etc.)
saveRDS(res_s3, file = paste0("output/bothdirec/results/N", N, "/results_scenario3_N", N, ".rds")) # save results
rm(data_s3, res_s3)


# Scenario 4
data_s4 <- readRDS(paste0("output/bothdirec/simdat/N", N, "/scenario4_N", N, ".rds"))
data_s4 <- analyze_data(data_s4, models = list(clpm = clpm, # analyze data
                                               ...
                                               dpm = dpm,
                                               dpm_free = dpm_free))
saveRDS(data_s4, file = paste0("output/bothdirec/analyzed_dat/N", N, "/analyzed_scenario4_N", N, ".rds")) # analyzed data
...
saveRDS(res_s4, file = paste0("output/bothdirec/results/N", N, "/results_scenario4_N", N, ".rds")) # save results
rm(data_s4, res_s4)

# Scenario 5
data_s5 <- readRDS(paste0("output/bothdirec/simdat/N", N, "/scenario5_N", N, ".rds"))
data_s5 <- analyze_data(data_s5, models = list(clpm = clpm, # analyze data
                                               ...
                                               dpm = dpm,
                                               dpm_free = dpm_free))
saveRDS(data_s5, file = paste0("output/bothdirec/analyzed_dat/N", N, "/analyzed_scenario5_N", N, ".rds")) # analyzed data
...
data_s5 <- remove_invalid(data_s5)
res_s5 <- output_measures(data_s5, xyvar = output_vars) # output measures (bias, mse etc.)
saveRDS(res_s5, file = paste0("output/bothdirec/results/N", N, "/results_scenario5_N", N, ".rds")) # save results
rm(data_s5, res_s5)

# Scenario 6
data_s6 <- readRDS(paste0("output/bothdirec/simdat/N", N, "/scenario6_N", N, ".rds"))
data_s6 <- analyze_data(data_s6, models = list(clpm = clpm, # analyze data
                                               ...
                                               dpm = dpm,
                                               dpm_free = dpm_free))
saveRDS(data_s6, file = paste0("output/bothdirec/analyzed_dat/N", N, "/analyzed_scenario6_N", N, ".rds")) # analyzed data
...
data_s6 <- remove_invalid(data_s6)
res_s6 <- output_measures(data_s6, xyvar = output_vars) # output measures (bias, mse etc.)
saveRDS(res_s6, file = paste0("output/bothdirec/results/N", N, "/results_scenario6_N", N, ".rds")) # save results
rm(data_s6, res_s6)
options(warn = oldw)
