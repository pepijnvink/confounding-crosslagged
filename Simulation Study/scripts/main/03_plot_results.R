...
for(i in 1:6){
  allres[[i]] <- readRDS(paste0("output/main/results/N", N, "/results_scenario", i, "_N", N, ".rds"))
  allres[[i]] <- bind_rows(allres[[i]]$output, .id = "model")
}
names(allres) <- paste("Scenario", c(1, 2, "3a", "4a", "5d", 6))
measures_all_df <- bind_rows(allres, .id = "Scenario") %>%
  mutate(Scenario = factor(Scenario, levels = paste("Scenario", c(1, 2, "3a", "4a", "5d", 6)), ordered = TRUE))

measures_bias <- measures_all_df %>%
  dplyr::select(c(model, variable, time, true, estimate, bias_estimate, bias_mcse, Scenario)) %>%
  mutate(model = factor(model,
         ...
         value = bias_estimate,
         mcse = bias_mcse) %>%
  dplyr::select(-c(bias_estimate, bias_mcse))
...
all_measures_long <- bind_rows(measures_bias,
                               measures_rmse,
                               measures_coverage,
                               measures_power) %>%
...
plot_bias <-  all_measures_long %>%
  dplyr::mutate(Measure = factor(Measure,
                                 levels = c("bias", "rmse", "coverage", "power"),
                                 labels = c("Bias", "RMSE", "Coverage Rate", "Power")),
  ...
  dplyr::filter(variable == outcome, Measure == "Bias") %>%
  ggplot(aes(y = value,
             x = time,
             color = model)) +
  geom_point(size = 2) +
...
plot_all
ggsave(filename = paste0("figures/main/png/", outcome, "/N", N, "_", outcome, ".png"), plot = plot_all, width = 9, height = 7, units = "in")
ggsave(filename = paste0("figures/main/svg/", outcome, "/N", N, "_", outcome, ".svg"), plot = plot_all, width = 9, height = 7, units = "in")
